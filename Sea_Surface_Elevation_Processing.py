...
data_admiralty_29hari.to_excel('/content/ADMIRALTY_12918030_rawdata.xlsx') # Edit nama filenya, dengan format, "ADMIRALTY_NIM.xlsx"


# In[31]:


data_admiralty_29hari.to_excel('/content/ADMIRALTY_12918030.xlsx') # Edit nama filenya, dengan format, "ADMIRALTY_NIM.xlsx"


# ### Tugas TAMBAHAN
# - Ganti frekuensi cut-off pada filter low pass dengan nilai 12 dan 24 jam
# - Lalu cari informasi statistika deskriptivenya antara raw data, filter cut-off 12 jam, dan filter cut-off 24 jam
# - buat overlay grafiknya
# - **LALU ANALISIS HASILNYA**
# 
# Notes:
# Skrip untuk mengerjakan tugas tambahan ini dibuat di cell dibawah ini

# In[32]:


# Buat skrip filter 12 dan 24 jam
import numpy as np
import matplotlib.pyplot as plt
from scipy import signal
...
fc_12 = 1/(3600*12) # Frekuensi cut-off = 12 jam, satuan 1/s atau Hz
fc_24 = 1/(3600*24) # Frekuensi cut-off = 24 jam, satuan 1/s atau Hz

w_12 = fc_12 / (fs / 2) # Normalisasi frekuensi
w_24 = fc_24 / (fs / 2) # Normalisasi frekuensi

b_12, a_12 = signal.butter(5, w_12, 'low') # desain filter untuk 'low pass'
b_24, a_24 = signal.butter(5, w_24, 'low') # desain filter untuk 'low pass'

elevasi_filter_12 = signal.filtfilt(b_12, a_12, data_1994_Jun['elevasi_m']) # implementasi filter ke data
elevasi_filter_24 = signal.filtfilt(b_24, a_24, data_1994_Jun['elevasi_m']) # implementasi filter ke data
...
# skrip untuk mencari statistika deskriptifnya
data_1994_Jun[['elevasi_m','elevasi filter 12 jam','elevasi filter 24 jam']].describe()


# In[35]:


# Buat skrip untuk membuat overlay plot hasilnya
fig = go.Figure()
...
fig.add_trace(go.Scatter(
    y = data_1994_Jun['elevasi filter 12 jam'],
    x = data_1994_Jun.index,
    line = dict(shape = 'spline'),
    name = 'Elevasi filter 12 jam [m]'
))

fig.add_trace(go.Scatter(
    y = data_1994_Jun['elevasi filter 24 jam'],
    x = data_1994_Jun.index,
    line = dict(shape = 'spline'),
    name = 'Elevasi filter 24 jam [m]'
))
fig.show()


# ### ANALISIS HASIL TUGAS TAMBAHAN
# Masukan analisisnya disini:
# 
# Jika dilihat dari grafik diatas terlihat bahwa grafik data sebelum di lakukan filter (warna biru) memiliki puncak yang lebih tinggi dibanding dengan grafik dari data yang sudah di filter. Karena filter yang digunakan adalah low-pass filter maka hanya yang memiliki frekuensi rendah yang lolos filter. Semakin besar cut-off nya makanya frekuensi nya akan semakin kecil karena kedua hal tersebut berbanding terbalik.
